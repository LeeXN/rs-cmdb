...
use common::entity::hardware::{RAM, RAMModule};
...
fn get_memory_info() -> io::Result<MemoryDevice> {
    ...
    let dmi = match fs::read("/sys/firmware/dmi/tables/DMI") {
        ...
        Err(err) => {
            ...
            return Err(Error::new(ErrorKind::Other, format!("Error reading DMI table: {}", err)));
        }
    };
            ...
            17 => {
                ...
                let memory_type = if p + 0x12 < dmi.len() {
                   ...
                   // ];
                    ...
                    let mem_types = ["Other", "Unknown", "DRAM", "EDRAM", "VRAM", "SRAM", "RAM", "ROM", "FLASH",
					"EEPROM", "FEPROM", "EPROM", "CDRAM", "3DRAM", "SDRAM", "SGRAM", "RDRAM",
					"DDR", "DDR2", "DDR2 FB-DIMM", "Reserved", "Reserved", "Reserved", "DDR3",
                ...
                let form_factor = if p + 0x0e < dmi.len() {
                    ...
                    let form_name = form_factor_map.iter()
                ...
                let serial_number = if p + 0x18 < dmi.len() {
                    ...
                    let sn = get_string_value(&dmi, p, serial_idx).unwrap_or_else(|| "Unknown".to_string());
                ...
                let part_number = if p + 0x1a < dmi.len() {
                    ...
                    let pn = get_string_value(&dmi, p, part_idx).unwrap_or_else(|| "Unknown".to_string());
                ...
                let device_locator = if p + 0x11 < dmi.len() {
                    ...
                    get_string_value(&dmi, p, device_locator_idx).unwrap_or_else(|| "Unknown".to_string())
                } else {
                    "Unknown".to_string()
                };
                ...
                memory_device.modules.push(ModuleInfo {
                    ...
                    memory_type,
                    form_factor,
                    locator: if device_locator != "Unknown" { device_locator } else { locator },
                });
            }
            19 => {
                ...
                if start == 0xffffffff && end == 0xffffffff {
                    ...
                    if rec_len >= 0x1f && p + 0x17 + 7 < dmi.len() {
                        ...
                        mem_size_alt += ((end64 - start64 + 1) / 1024 / 1024) as u32; // 转换为 MB
                    }
                } else {
    ...
    debug!("Memory info: Type={}, Size={}MB, Speed={}MHz, Count={}", 
           memory_device.memory_type, memory_device.total_size, 
           memory_device.speed, memory_device.number);
...
pub fn collect_ram() -> RAM {
    match get_memory_info() {
        Ok(memory) => {
            ...
            let modules = memory.modules.iter().map(|m| {
                RAMModule {
...
fn is_valid_string(s: &str) -> bool {
    let s = s.trim();
    !(s.is_empty() || s == "0" || s == "00000000" || s.eq_ignore_ascii_case("unknown") || s.eq_ignore_ascii_case("not specified"))
}
...
mod tests {
    ...
    fn test_get_string_value() {
        ...
        assert_eq!(get_string_value(&dmi_data, 0, 1), Some("String1".to_string()));
        assert_eq!(get_string_value(&dmi_data, 0, 2), Some("String2".to_string()));
