...
use common::entity::hardware::{NIC, NICType, NICStatus};
...
const IGNORE_IFACES: [&str; 11] = ["cali", "kube-ipvs", "nodelocaldns", "tunl0", "tap0", "veth", "docker", "virbr", "br-", "vxlan", "lo"];
...
impl SystemCache {
    ...
    fn get_ipv4_gateway(&self, interface: &str) -> Option<String> {
        self.ipv4_routes
            .iter()
            .find(|route| route.interface == interface && route.is_default && route.gateway != "00000000")
            .and_then(|route| {
                if let Ok(gw_val) = u32::from_str_radix(&route.gateway, 16) {
                    let ip_bytes = gw_val.to_le_bytes();
                    let gateway_ip = Ipv4Addr::new(ip_bytes[0], ip_bytes[1], ip_bytes[2], ip_bytes[3]);
    ...
    fn get_ipv6_gateway(&self, interface: &str) -> Option<String> {
        self.ipv6_routes
            .iter()
            .find(|route| route.interface == interface && route.is_default && route.gateway != "00000000000000000000000000000000")
            .and_then(|route| {
                let mut addr_bytes = [0u8; 16];
                for i in 0..16 {
        ...
        for addr_info in &self.ipv6_addresses {
            if addr_info.interface == interface {
                if !addr_info.address.starts_with("fe80") {
                    return (addr_info.address.clone(), format!("/{}", addr_info.prefix_len));
                } else if link_local.is_none() {
                    link_local = Some((addr_info.address.clone(), format!("/{}", addr_info.prefix_len)));
                }
            }
        }
    ...
    fn parse_pci_ids(&self, vendor_id: &str, device_id: &str) -> Option<(String, String)> {
        ...
        for line in content.lines() {
            ...
            } else if line.starts_with('\t') && !line.starts_with("\t\t") && current_vendor == vendor_id {
                let line = line.trim();
                let parts: Vec<&str> = line.splitn(2, ' ').collect();
                if parts.len() >= 2 {
...
impl SysfsInterface {
    ...
    fn read_value(&self, file: &str) -> io::Result<String> {
        fs::read_to_string(self.base_path.join(file))
...
// 获取接口基本信息
fn get_interface_basic_info(sysfs: &SysfsInterface) -> io::Result<(String, NICStatus, u32, Vec<String>)> {
    let mac_address = sysfs.read_value("address").unwrap_or_else(|_| "Unknown".to_string());
    let operstate = sysfs.read_value("operstate").unwrap_or_else(|_| "unknown".to_string());
    let speed_str = sysfs.read_optional_value("speed").unwrap_or_else(|| "0".to_string());
...
fn get_device_info(sysfs: &SysfsInterface, cache: &SystemCache) -> InterfaceDeviceInfo {
    ...
    if let Some(device_path) = sysfs.get_device_path() {
        ...
        if info.vendor_id.is_empty() {
            info.vendor_id = sysfs.read_optional_value("device/vendor")
                .unwrap_or_else(|| "Unknown".to_string());
        }
        if info.device_id.is_empty() {
            info.device_id = sysfs.read_optional_value("device/device")
                .unwrap_or_else(|| "Unknown".to_string());
        }

        // 获取友好名称
        if let Some((vendor_name, device_name)) = cache.parse_pci_ids(&info.vendor_id, &info.device_id) {
            info.vendor_name = format!("{}(0x{})", vendor_name, info.vendor_id.trim_start_matches("0x"));
            info.device_name = format!("{}(0x{})", device_name, info.device_id.trim_start_matches("0x"));
        } else {
            // 使用预定义的厂商名称
            info.vendor_name = match info.vendor_id.trim_start_matches("0x") {
                ...
                _ => format!("Unknown Vendor(0x{})", info.vendor_id.trim_start_matches("0x"))
            };
            info.device_name = format!("Unknown Device(0x{})", info.device_id.trim_start_matches("0x"));
        }
    } else {
...
// 获取IP地址信息
fn get_ip_info(interface: &str, cache: &SystemCache) -> (String, String, String, String, String, String) {
            ...
            for line in output_str.lines() {
                if line.trim().starts_with("inet6 ") {
                    ...
                    if parts.len() >= 2 {
                        let inet_parts: Vec<&str> = parts[1].split('/').collect();
                        if inet_parts.len() == 2 {
                            if !inet_parts[0].starts_with("fe80") || ipv6_address.is_empty() {
    ...
    (ipv4_address, ipv4_subnet_mask, ipv4_gateway, ipv6_address, ipv6_subnet_mask, ipv6_gateway)
}
...
fn list_interfaces() -> io::Result<Vec<String>> {
    ...
    for line in reader.lines().skip(2) {
        ...
        if let Some(iface_part) = line.split(':').next() {
            ...
            // 过滤掉不需要的接口
            if !IGNORE_IFACES.iter().any(|prefix| iface_part.starts_with(prefix)) {
...
pub fn collect_nics() -> io::Result<Vec<NIC>> {
    ...
    for interface in interfaces {
        ...
        // 获取IP信息
        let (ipv4_address, ipv4_subnet_mask, ipv4_gateway, ipv6_address, ipv6_subnet_mask, ipv6_gateway) = 
        ...
        nics.push(NIC {
            name: interface,
            vendor: if device_info.vendor_name.is_empty() { "Unknown".to_string() } else { device_info.vendor_name },
            model: if device_info.device_name.is_empty() { "Unknown".to_string() } else { device_info.device_name },
            speed,
            mac_address,
            ipv4_address,
            ipv4_subnet_mask,
            ...
            bonding_slaves,
            status,
            pci_slot: if device_info.pci_slot.is_empty() { None } else { Some(device_info.pci_slot) },
