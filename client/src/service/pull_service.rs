...
impl PullService {
    ...
    pub async fn handle_pull_request(&self, request: PullRequest) -> Result<()> {
        ...
        let response_url = format!(
            "{}/clients/{}/hardware/pull/response",
            self.config.server.url, self.client_id
        );

        let result = client.post(&response_url).json(&response).send().await?;
    ...
    pub async fn check_pending_requests(&self) -> Result<()> {
        ...
        let pending_url = format!(
            "{}/clients/{}/hardware/pull/pending",
            self.config.server.url, self.client_id
        );

        let response = client.get(&pending_url).send().await?;
