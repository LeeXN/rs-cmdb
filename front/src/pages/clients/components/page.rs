...
pub fn clients_page() -> Html {
            ...
            api::get_filter_options(Callback::from({
                ...
                move |result: Result<FilterOptions, ApiError>| {
                    if let Ok(options) = result {
                        state.dispatch(ClientsAction::SetFilterOptions(options))
                    }
                }
            }));
    ...
    let perform_export = {
        ...
        std::rc::Rc::new(move |clients: &Vec<Client>| {
            ...
            let _statuses = ["Active", "Maintenance", "InStock", "Decommissioned"];
            let _environments = ["Prod", "Dev", "Test", "Staging"];

            // Write Data
            for (row, client) in clients.iter().enumerate() {
                let r = (row + 1) as u32;

                let rack_name = client
                    .rack
                    .as_ref()
                ...
                let _ = worksheet.write_string(
                    r,
                    11,
                    client.warranty_expiration.clone().unwrap_or_default(),
                );
    ...
    let on_export_selected_click = {
        ...
        let perform_export = perform_export.clone();
        Callback::from(move |_: MouseEvent| {
            let selected: Vec<Client> = all_clients
                .iter()
                        ...
                        if let Ok(range) = excel.worksheet_range("Sheet1") {
                            ...
                            for (i, row) in range.rows().skip(1).enumerate() {
                                ...
                                if let Some(client) = all_clients.iter().find(|c| c.id == id) {
                                    ...
                                    let get_int = |idx| {
                                        row.get(idx)
                                            .and_then(|c: &calamine::Data| c.as_f64())
                                            .map(|f| f as u32)
                                            .or_else(|| {
                                                row.get(idx)
                                                    .and_then(|c: &calamine::Data| c.as_string())
                                    ...
                                    if !rack_name.is_empty() {
                                        if let Some(rack) =
                                            racks.iter().find(|r| r.name == rack_name)
                                        {
                                            ...
                                            if let Some(p) = power {
                                                ...
                                                if let Some(limit) = rack.power_limit {
                                                    let current_usage: u32 = all_clients
                                                        .iter()
                                                        .filter(|c| {
                                                            c.rack == Some(rack.id.clone())
                                                                && c.id != client.id
                                    ...
                                    if !owner_name.is_empty() {
                                        if let Some(person) =
                                            persons.iter().find(|p| p.name == owner_name)
                                        {
                                    ...
                                    if !status_str.is_empty() {
                                        match status_str.as_str() {
                                            "Active" => {
                                                updated_client.status = Some(ClientStatus::Active)
                                            }
                                            "Maintenance" => {
                                                updated_client.status =
                                                    Some(ClientStatus::Maintenance)
                                            }
                                            "InStock" => {
                                                updated_client.status = Some(ClientStatus::InStock)
                                            }
                                            "Decommissioned" => {
                                                updated_client.status =
                                                    Some(ClientStatus::Decommissioned)
                                            }
                                    ...
                                    if !env_str.is_empty() {
                                        match env_str.as_str() {
                                            "Prod" => {
                                                updated_client.environment = Some(Environment::Prod)
                                            }
                                            "Dev" => {
                                                updated_client.environment = Some(Environment::Dev)
                                            }
                                            "Test" => {
                                                updated_client.environment = Some(Environment::Test)
                                            }
                                            "Staging" => {
                                                updated_client.environment =
                                                    Some(Environment::Staging)
                                            }
                                            _ => errors.push(format!(
                                                "Row {}: Invalid Environment '{}'",
                                                row_num, env_str
                                            )),
                                        }
                                    }
                                    ...
                                    if !warranty.is_empty() {
                                        updated_client.warranty_expiration = Some(warranty);
                                    }
                                    if !asset_tag.is_empty() {
                                        updated_client.asset_tag = Some(asset_tag);
                                    }
                                    if !supplier.is_empty() {
                                        updated_client.supplier = Some(supplier);
                                    }
                                    if !comment.is_empty() {
                                        updated_client.comment = Some(comment);
                                    }
