...
use common::models::{
    CastCleanupRequest, CastCleanupResponse,
    CommandTaskListResponse, CreateCommandRequest, CreateCommandResponse,
    CreateTerminalSessionRequest, CreateTerminalSessionResponse, PaginatedResult,
    RemoteExecConfigResponse, RemoteExecOpsOverviewResponse, ResizeTerminalRequest,
    TerminalInputRequest, TerminalOpsSummaryResponse, TerminalSessionSummary,
    UpdateRemoteExecConfigRequest,
};
...
pub async fn fetch_remote_exec_config() -> Result<RemoteExecConfigResponse, ApiError> {
    ...
    match get(&url).await {
        Ok(resp) if resp.status() == 200 => resp
            ...
            .and_then(|r| r.data.ok_or(ApiError { message: "no data".into() })),
...
) -> Result<CreateCommandResponse, ApiError> {
...
pub async fn list_terminal_sessions(client_id: &str) -> Result<Vec<TerminalSessionSummary>, ApiError> {
    let url = format!("{}?client_id={}", TERMINAL_BASE, urlencoding::encode(client_id));
    match get(&url).await {
        Ok(resp) if resp.status() == 200 => resp
...
pub fn set_remote_exec_config(enabled: bool, cb: Callback<Result<RemoteExecConfigResponse, ApiError>>) {
    spawn_local(async move { cb.emit(update_remote_exec_config(enabled).await) });
}

pub fn submit_command(req: CreateCommandRequest, cb: Callback<Result<CreateCommandResponse, ApiError>>) {
...
pub fn list_commands(
    ...
    spawn_local(async move {
        cb.emit(fetch_commands(client_id.as_deref(), page, page_size).await)
    });
}
