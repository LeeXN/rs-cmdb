...
impl ExecutionSessionService {
    ...
    pub async fn list_sessions(&self, query: &CommandQuery) -> CmdbResult<PaginatedResult<ExecutionSession>> {
        let query = self.resolve_client_filter(query).await?;
        self.repo.list_all(&query).await
    }

    pub async fn list_user_sessions(&self, user_id: &str, query: &CommandQuery) -> CmdbResult<PaginatedResult<ExecutionSession>> {
        let query = self.resolve_client_filter(query).await?;
    ...
    pub async fn get_session(
        &self,
        session_id: &str,
    ) -> CmdbResult<Option<ExecutionSession>> {
    ...
    async fn resolve_client_filter(&self, query: &CommandQuery) -> CmdbResult<CommandQuery> {
        ...
        next.client_id = matched_ids.first().cloned().or_else(|| Some("__no_match__".to_string()));
        Ok(next)
    }
}
...
mod tests {
    ...
    fn client(id: &str, hostname: &str, ip: &str, primary_ip: Option<&str>, serial: Option<&str>) -> Client {
        Client {
            id: id.to_string(),
    ...
    fn client_matches_checks_all_supported_identifier_fields() {
        let client = client("client-77", "db-prod-01", "10.1.0.8", Some("172.16.8.9"), Some("SN-7788"));
    ...
    fn matching_client_ids_returns_all_matches_in_input_order() {
        let clients = vec![
            ...
            client("client-2", "web-b", "10.0.0.2", Some("172.16.0.2"), Some("SN-2")),
            client("client-3", "db-a", "10.0.0.3", None, Some("SN-3")),
        ];

        assert_eq!(matching_client_ids(&clients, "web"), vec!["client-1".to_string(), "client-2".to_string()]);
        assert_eq!(matching_client_ids(&clients, "SN-3"), vec!["client-3".to_string()]);
