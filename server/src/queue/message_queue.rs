...
mod tests {
    ...
    async fn test_multiple_messages() {
        ...
        queue.send_message(Message::ClientHeartbeat("third".into())).unwrap();

        let msg1 = queue.receive_message(Duration::from_secs(1)).unwrap().unwrap();
        let msg2 = queue.receive_message(Duration::from_secs(1)).unwrap().unwrap();
        let msg3 = queue.receive_message(Duration::from_secs(1)).unwrap().unwrap();
