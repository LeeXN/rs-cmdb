    ...
    pub fn new<P: AsRef<Path>>(path: P) -> CmdbResult<Self> {
        ...
        // Initialize the table to ensure it exists (synchronous)
        {
            let write_txn = db.begin_write()
                .map_err(|e| CmdbError::Database(format!("Failed to start init transaction: {}", e)))?;
            
            // Simply open and close the table to ensure it gets created
            {
                let _table = write_txn.open_table(KV_TABLE)
                    .map_err(|e| CmdbError::Database(format!("Failed to initialize table: {}", e)))?;
            }
            
            write_txn.commit()
                .map_err(|e| CmdbError::Database(format!("Failed to commit init transaction: {}", e)))?;
        }
...
impl Database for RedbStore {
    async fn set(&self, key: &str, value: &[u8]) -> CmdbResult<()> {
        ...
        tokio::task::spawn_blocking(move || {
            let write_txn = db.begin_write()
                .map_err(|e| CmdbError::Database(format!("Failed to start write transaction: {}", e)))?;
            
            {
                ...
                table.insert(key.as_str(), value.as_slice())
                    .map_err(|e| CmdbError::Database(format!("Failed to set key {}: {}", key, e)))?;
            }
            ...
            Ok(())
        }).await.map_err(|e| CmdbError::Database(format!("Task join error: {}", e)))?
    }
    ...
    async fn get(&self, key: &str) -> CmdbResult<Option<Vec<u8>>> {
        ...
        tokio::task::spawn_blocking(move || {
            let read_txn = db.begin_read()
                .map_err(|e| CmdbError::Database(format!("Failed to start read transaction: {}", e)))?;
            ...
            let result = match table.get(key.as_str()) {
                ...
                Ok(None) => Ok(None),
                Err(e) => Err(CmdbError::Database(format!("Failed to get key {}: {}", key, e))),
            };
        ...
        tokio::task::spawn_blocking(move || {
            ...
            
            {
                ...
                let _ = table.remove(key.as_str())
                    .map_err(|e| CmdbError::Database(format!("Failed to delete key {}: {}", key, e)))?;
            }
    ...
    async fn list_keys(&self, prefix: &str) -> CmdbResult<Vec<String>> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            
            {
                ...
                for item in iter {
                    let (key, _) = item.map_err(|e| CmdbError::Database(format!("Failed to iterate: {}", e)))?;
    ...
    async fn list_values(&self, prefix: &str) -> CmdbResult<Vec<Vec<u8>>> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            
            {
                ...
                for item in iter {
                    let (key, value) = item.map_err(|e| CmdbError::Database(format!("Failed to iterate: {}", e)))?;
                    let key_str = key.value();
                    if key_str.starts_with(&prefix) {
