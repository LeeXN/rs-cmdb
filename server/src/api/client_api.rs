...
use crate::repository::{client_repository::ClientRepository, hardware_repository::HardwareRepository, component_repository::ComponentRepository};
        ...
        Ok(mut clients) => {
            ...
            if let Some(ref search) = query.search {
                ...
                clients.retain(|c| {
                    ...
                    c.os.as_ref().map_or(false, |os| os.to_lowercase().contains(&search_lower))
                });
            }
            ...
            if let Some(ref os) = query.os {
                if os != "all" {
                    clients.retain(|c| c.os.as_ref().map_or(false, |o| o == os));
                }
            }
            ...
            if let Some(ref status) = query.status {
                if status != "all" {
                    ...
                    clients.retain(|c| {
                        let is_online = c.last_seen.as_ref()
            ...
            info!("Listed {} clients (page {}/{})", items.len(), page, total_pages);
...
pub async fn register_client(
    ...
    if let Some(rack_id) = &registration.rack {
        if !rack_id.is_empty() {
            if let Err(e) = validation_service.validate_rack_exists(rack_id).await {
                let response = ApiResponse::<Client> {
    ...
    if let Some(project_id) = &registration.project_id {
        if !project_id.is_empty() {
            if let Err(e) = validation_service.validate_project_exists(project_id).await {
                let response = ApiResponse::<Client> {
                    status: e.status_code(),
                    message: e.to_string(),
                    data: None,
    ...
    if let Some(owner_id) = &registration.owner_id {
        if !owner_id.is_empty() {
            if let Err(e) = validation_service.validate_person_exists(owner_id).await {
                let response = ApiResponse::<Client> {
                    status: e.status_code(),
                    message: e.to_string(),
                    data: None,
    ...
    if let Err(err) = message_queue.send_message(Message::ClientRegistration(registration.clone())) {
        error!("Failed to queue registration for {}: {}", registration.hostname, err);
        let response = ApiResponse::<Client> {
            status: err.status_code(),
    ...
    match client_service.register_client(
        ...
        Err(err) => {
            error!("Failed to register client {}: {}", registration.hostname, err);
            let response = ApiResponse::<Client> {
                status: err.status_code(),
                message: err.to_string(),
                data: None,
...
pub async fn delete_client(
    ...
    if let Err(e) = component_repo.release_components_by_client(&client_id).await {
         let response = ApiResponse::<()> {
            status: 500,
            ...
            for client in clients {
                ...
                let matches_search = if let Some(ref search_term) = params.q {
                    if search_term.is_empty() {
                        ...
                        let client_matches = client.hostname.to_lowercase().contains(&search_lower)
                            ...
                            || client.sys_vendor.as_ref().map_or(false, |v| v.to_lowercase().contains(&search_lower))
                            || client.product_name.as_ref().map_or(false, |p| p.to_lowercase().contains(&search_lower))
                            || client.serial_number.as_ref().map_or(false, |s| s.to_lowercase().contains(&search_lower));
                ...
                let matches_os = if let Some(ref os_filter) = params.os {
                    if os_filter == "all" {
                        ...
                        client.os.as_ref().map_or(false, |os| os == os_filter)
                    }
                } else {
                    true
                };
            ...
            for client in clients {
                ...
                let matches_kernel = if let Some(ref kernel_filter) = params.os_kernel_filter {
                    if kernel_filter.is_empty() || kernel_filter == crate::constants::FILTER_ALL {
                        ...
                        client.kernel_version.as_ref().map_or(false, |k| k == kernel_filter)
                    }
                } else {
                    true
                };
                ...
                let matches_server_vendor = if let Some(ref vendor_filter) = params.server_vendor_filter {
                    if vendor_filter.is_empty() || vendor_filter == crate::constants::FILTER_ALL {
                        ...
                        client.sys_vendor.as_ref().map_or(false, |vendor| vendor == vendor_filter)
                    }
                } else {
                    true
                };
                ...
                let matches_server_model = if let Some(ref model_filter) = params.server_model_filter {
                    if model_filter.is_empty() || model_filter == crate::constants::FILTER_ALL {
                        ...
                        client.product_name.as_ref().map_or(false, |model| model == model_filter)
                    }
                } else {
                    true
                };
                ...
                let matches_client_status = if let Some(ref status_filter) = params.client_status_filter {
                    if status_filter.is_empty() || status_filter == crate::constants::FILTER_ALL {
                        ...
                        client.status.as_ref().map_or(false, |s| format!("{:?}", s) == *status_filter)
                    }
                } else {
                    true
                };
                ...
                let matches_environment = if let Some(ref env_filter) = params.environment_filter {
                    if env_filter.is_empty() || env_filter == crate::constants::FILTER_ALL {
                        ...
                        client.environment.as_ref().map_or(false, |e| format!("{:?}", e) == *env_filter)
                    }
                } else {
                    true
                };
                ...
                let matches_rack = if let Some(ref rack_filter) = params.rack_id_filter {
                    if rack_filter.is_empty() || rack_filter == crate::constants::FILTER_ALL {
                        ...
                        client.rack.as_ref().map_or(false, |r| r == rack_filter)
                    }
                } else {
                    true
                };
                ...
                let matches_project = if let Some(ref project_filter) = params.project_id_filter {
                    if project_filter.is_empty() || project_filter == crate::constants::FILTER_ALL {
                        ...
                        client.project_id.as_ref().map_or(false, |p| p == project_filter)
                    }
                } else {
                    true
                };
                ...
                let matches_owner = if let Some(ref owner_filter) = params.owner_id_filter {
                    if owner_filter.is_empty() || owner_filter == crate::constants::FILTER_ALL {
                        ...
                        client.owner_id.as_ref().map_or(false, |o| o == owner_filter)
                    }
                } else {
                    true
                };
                ...
                let matches_hardware = if let Some(ref hw) = hardware {
                    ...
                    let matches_network_type = if let Some(ref network_type_filter) = params.network_type_filter {
                        if network_type_filter.is_empty() || network_type_filter == crate::constants::FILTER_ALL {
                            ...
                            hw.nics.iter().any(|nic| nic.nic_type.to_string() == *network_type_filter)
                        }
                    } else {
                        true
                    };
                    ...
                    let matches_storage = if let Some(ref storage_type_filter) = params.storage_type_filter {
                        if storage_type_filter.is_empty() || storage_type_filter == crate::constants::FILTER_ALL {
                            ...
                            hw.disks.iter().any(|disk| disk.storage_type.to_string() == *storage_type_filter)
                        }
                    } else {
                        true
                    };
                    
                    matches_cpu_vendor && matches_cpu_model && matches_gpu_vendor && 
                    matches_gpu_model && matches_memory && matches_network_type && 
                    matches_network_model && matches_storage
                } else {
                    ...
                    let has_hardware_filters = params.cpu_vendor_filter.is_some() || params.cpu_model_filter.is_some() ||
                        params.gpu_vendor_filter.is_some() || params.gpu_model_filter.is_some() ||
                        params.memory_min_filter.is_some() || params.memory_max_filter.is_some() ||
                        params.network_type_filter.is_some() || params.network_model_filter.is_some() ||
                ...
                if matches_search && matches_os && matches_kernel && matches_server_vendor && 
                   matches_server_model && matches_hardware && matches_status && matches_client_status &&
                   matches_environment && matches_rack && matches_project && matches_owner {
                    filtered_clients.push(client);
                }
            }
            
            info!("Hardware filter returned {} clients", filtered_clients.len());
            let response = ApiResponse {
                status: 200,
...
pub async fn get_filter_options_by_client_ids(
    ...
    use common::models::{FilterOptions, ApiResponse};
