...
use crate::service::{
    ...
    client_service::ClientService, command_service::CommandService, export_service::ExportService,
...
pub fn create_router(
    ...
    let cors_origins: Vec<axum::http::HeaderValue> = config
        ...
        .collect();
    // Invalid or empty origin configuration must fail closed. Falling back
    // to `*` here would silently turn a deployment typo into a cross-origin
    ...
    let public_routes = Router::new()
        ...
        .route("/upgrade.sh", get(download_api::get_upgrade_script))
    ...
    let agent_routes = Router::new()
        .route(
            "/api/v1/agent/commands/pending",
            get(agent_command_api::poll_pending),
        )
        .route(
            "/api/v1/agent/commands/{id}/start",
            post(agent_command_api::start_command),
        )
        .route(
            "/api/v1/agent/commands/{id}/logs",
            post(agent_command_api::push_logs),
        )
        .route(
            "/api/v1/agent/commands/{id}/complete",
            post(agent_command_api::complete_command),
        )
        .route(
            "/api/v1/agent/terminal-sessions/stream",
            get(agent_terminal_api::stream_terminal),
        )
        .route(
            "/api/v1/agent/terminal-sessions/pending",
            get(agent_terminal_api::poll_pending),
        )
        .route(
            "/api/v1/agent/terminal-sessions/{id}/output",
            post(agent_terminal_api::push_output),
        )
        .route(
            "/api/v1/agent/terminal-sessions/{id}/state",
            post(agent_terminal_api::report_state),
        )
    ...
    let admin_routes = Router::new()
        ...
        .route(
            "/api/v1/clients/{id}/agent-token/recover",
            post(client_api::recover_agent_token),
        )
        .route(
            "/api/v1/accounts/{id}",
        ...
        .route(
            ...
            put(command_api::update_config),
        )
        // Exec policy CRUD (Admin only)
        .route(
            "/api/v1/permissions/exec-policies",
            get(exec_policy_api::list_exec_policies).post(exec_policy_api::create_exec_policy),
        )
        .route(
            "/api/v1/permissions/exec-policies/{id}",
            get(exec_policy_api::get_exec_policy)
                .put(exec_policy_api::update_exec_policy)
                .delete(exec_policy_api::delete_exec_policy),
        )
        .route(
            "/api/v1/permissions/overview",
            get(permission_admin_api::get_permission_overview),
        )
        .route(
            "/api/v1/permissions/rules",
            get(permission_admin_api::list_rules).post(permission_admin_api::create_rule),
        )
        .route(
            "/api/v1/permissions/rules/{id}",
            get(permission_admin_api::get_rule)
                .put(permission_admin_api::update_rule)
                .delete(permission_admin_api::delete_rule),
        )
        .route(
            "/api/v1/permissions/groups",
            get(permission_admin_api::list_groups).post(permission_admin_api::create_group),
        )
        .route(
            "/api/v1/permissions/groups/{id}",
            get(permission_admin_api::get_group)
                .put(permission_admin_api::update_group)
                .delete(permission_admin_api::delete_group),
        )
        // Web terminal policy CRUD (Admin only)
        .route(
            "/api/v1/permissions/web-terminal-policies",
            get(web_terminal_policy_api::list_web_terminal_policies)
                .post(web_terminal_policy_api::create_web_terminal_policy),
        )
        .route(
            "/api/v1/permissions/web-terminal-policies/{id}",
            get(web_terminal_policy_api::get_web_terminal_policy)
                .put(web_terminal_policy_api::update_web_terminal_policy)
                .delete(web_terminal_policy_api::delete_web_terminal_policy),
        )
        // Approval management (Admin only)
        .route(
            "/api/v1/permissions/pending-approvals",
            get(approval_api::list_pending_approvals),
        )
        .route(
            "/api/v1/permissions/approval-summary",
            get(approval_api::get_approval_summary),
        )
        .route(
            "/api/v1/permissions/pending-approvals/{id}",
            get(approval_api::get_pending_approval),
        )
        .route(
            "/api/v1/permissions/pending-approvals/{id}/approve",
            post(approval_api::approve_request),
        )
        .route(
            "/api/v1/permissions/pending-approvals/{id}/reject",
            post(approval_api::reject_request),
        )
        .route(
            "/api/v1/remote-exec/ops/overview",
            get(remote_exec_ops_api::get_overview),
        )
        .route(
            "/api/v1/remote-exec/terminal-ops/summary",
            get(remote_exec_ops_api::get_terminal_summary),
        )
        .route(
            "/api/v1/remote-exec/terminal-sessions/all",
            get(remote_exec_ops_api::list_terminal_sessions),
        )
        .route(
            "/api/v1/remote-exec/casts/cleanup",
            post(remote_exec_ops_api::cleanup_casts),
        )
        .route_layer(middleware::from_fn(rbac::require_admin));
    ...
    let admin_write_routes = Router::new()
        .route(
            ...
            post(command_api::create_command),
        )
        .route_layer(middleware::from_fn(rbac::require_admin));
    ...
    let read_routes = Router::new()
        ...
        .route(
            ...
            get(approval_api::list_my_approvals),
        )
        ...
        .route(
            "/api/v1/remote-exec/terminal-sessions/{id}",
            get(terminal_session_api::get_session).delete(terminal_session_api::close_session),
        )
        .route(
            "/api/v1/remote-exec/terminal-sessions/{id}/input",
            post(terminal_session_api::push_input),
        )
        .route(
            "/api/v1/remote-exec/terminal-sessions/{id}/resize",
            post(terminal_session_api::resize_session),
        )
        .route(
            "/api/v1/remote-exec/terminal-sessions/{id}/stream",
            get(terminal_session_api::stream_session),
        )
        .route(
            "/api/v1/remote-exec/terminal-sessions/{id}/ws",
            get(terminal_session_api::websocket_session),
        )
        // Remote exec: read-only routes (all authenticated)
        .route("/api/v1/remote-exec/config", get(command_api::get_config))
        .route(
            "/api/v1/remote-exec/commands",
            get(command_api::list_commands),
        )
        .route(
            "/api/v1/remote-exec/commands/{id}",
            get(command_api::get_command),
        )
        .route(
            "/api/v1/remote-exec/commands/{id}/logs",
            get(command_api::get_command_logs),
        )
        .route(
            "/api/v1/remote-exec/commands/{id}/stream",
            get(command_api::stream_command),
        )
