...
pub async fn get_permission_overview(
    ...
    let rules_count = match perm_repo.rule_count().await {
        ...
        Err(e) => {
            ...
            return error_response::<PermissionOverviewResponse>(
                e.status_code(),
...
pub async fn list_rules(
    ...
    match perm_svc.list_rules().await {
        Ok(rules) => success_response(
            StatusCode::OK,
            "Permission rules retrieved successfully",
            rules,
        ),
        Err(e) => {
...
pub async fn get_rule(
    ...
    match perm_svc.get_rule(&id).await {
        Ok(Some(rule)) => success_response(
            StatusCode::OK,
            "Permission rule retrieved successfully",
            rule,
        ),
        Ok(None) => {
            error_response::<PermissionRule>(404, &format!("Permission rule {} not found", id))
        }
        Err(e) => {
...
pub async fn create_rule(
    ...
    let Json(rule) = match result {
        ...
        Err(rejection) => {
            ...
            return error_response::<PermissionRule>(
                422,
    ...
    match perm_svc.create_rule(&rule).await {
        Ok(()) => {
            ...
            success_response(
                StatusCode::CREATED,
                "Permission rule created successfully",
                rule,
            )
        }
        Err(e) => {
...
pub async fn get_group(
    ...
    match perm_svc.get_group(&id).await {
        Ok(Some(group)) => success_response(
            StatusCode::OK,
