...
use common::entity::hardware::{Hardware, CPU, GPU, RAM, Disk, NIC, IpmiInfo, SystemInfo, OS};
use common::models::{Client, PullResponse, Rack, Project, Person};
...
/// ```
    ...
    pub fn create_test_client(id: &str) -> Client {
        Client {
            ...
            ip_address: format!("192.168.1.{}", id.split('-').last().unwrap_or("1")),
    ...
    pub fn create_test_hardware_info(client_id: &str) -> Hardware {
        Hardware {
            ...
            os: OS {
                ...
                ip_address: format!("192.168.1.{}", client_id.split('-').last().unwrap_or("1")),
            },
            cpu: CPU {
            ...
            nics: vec![
                NIC {
                    ...
                    speed: 1000,
                    mac_address: format!("00:11:22:33:44:{}", client_id.split('-').last().unwrap_or("55")),
                    ipv4_address: format!("192.168.1.{}", client_id.split('-').last().unwrap_or("1")),
    ...
    pub fn create_pull_response(client_id: &str, status: &str) -> PullResponse {
        PullResponse {
            ...
            hardware: if status == "success" { Some(create_test_hardware_info(client_id)) } else { None },
            error: if status == "error" { Some("Error message".to_string()) } else { None },
        }
    }
    ...
    pub fn create_test_person(id: &str) -> Person {
        Person {
            ...
            phone: Some(format!("123-456-7{}0", id.split('-').last().unwrap_or("0"))),
